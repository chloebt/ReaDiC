...
* **-e** HEADRULES : liste de règles de percolation des têtes, fichier head_rules.txt dans ReaDiC/data/ressources/ ;
...
##### Options (ou arguments dans l'ordre si fichier jar utilisé seul) :
  * **-m** CONNECTIVES : liste de tous les connecteurs du PDTB, fichier connective_pdtb.txt dans ReaDiC/data/ressources/ ;
...
Lit les fichiers en sortie de l'étape précédente et écrit en sortie des fichiers de traits. Certaines options nécessitent une valeur explicite (req) et d'autres ont une valeur par défaut (not req:default).
...
* **-b** JAVAOUT (req) : le répertoire contenant les fichiers en sortie des 3 étapes précédentes ;
* **-o** OUT  (not req:~/readCorpus_out/) : répertoires où seront créés les répertoires et fichiers en sortie (décrits ci-dessous) ;
* **-l** LABELS (req) : jeu de relations, fichier avec correspondance relation et label numérique ;
...
* **-r** RESSOURCES (not req:ReaDiC/data/ressources/) : répertoire contenant des lexiques utilisés pour calculer certains traits (classes de Levin, lexique de subjectivité, Inquirer) ;
